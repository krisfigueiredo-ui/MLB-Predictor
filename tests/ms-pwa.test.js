...
describe("shell asset list", () => {
  it("uses relative paths so the app works from a project subpath", () => {
    // GitHub Pages serves this repo at /MLB-Predictor/, not the domain root.
  ...
  it("covers every script the dashboard loads", () => {
    ["ms-app.js", "ms-leagues.js", "ms-espn.js", "ms-ratings.js", "ms-features.js",
     ...
     "ms-pwa.js", "ml-train.js", "calibration.js", "betting.js"].forEach(f => {
      expect(SHELL_ASSETS.some(a => a.endsWith("/" + f))).toBe(true);
    });
  });

  it("includes both pages, the manifest and the icons", () => {
    expect(SHELL_ASSETS).toContain("./sports.html");
...
describe("classifyRequest", () => {
  ...
  it("treats our own static assets as shell", () => {
    expect(classifyRequest(self + "/MLB-Predictor/sports.html", { selfOrigin: self })).toBe("shell");
    expect(classifyRequest(self + "/MLB-Predictor/js/ms-app.js", { selfOrigin: self })).toBe("shell");
    expect(classifyRequest(self + "/MLB-Predictor/manifest.webmanifest", { selfOrigin: self })).toBe("shell");
    expect(classifyRequest(self + "/MLB-Predictor/", { selfOrigin: self })).toBe("shell");
    expect(classifyRequest(self + "/MLB-Predictor/icons/icon-192.png", { selfOrigin: self })).toBe("shell");
  });

  it("ignores query strings when deciding", () => {
    expect(classifyRequest(self + "/MLB-Predictor/sports.html?tab=live", { selfOrigin: self })).toBe("shell");
  });
...
describe("relative URLs", () => {
  it("treats a relative same-origin asset as shell, not cross-origin", () => {
    ...
    expect(classifyRequest("/MLB-Predictor/sports.html", { selfOrigin: "https://example.github.io" })).toBe("shell");
  });
});
