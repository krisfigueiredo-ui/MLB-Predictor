(function(root) {
  ...
  function renderSlate() {
    ...
    rootEl.innerHTML = '<div class="ql-page-head"><div><div class="ql-eyebrow">'+games().length+' GAME'+(games().length === 1 ? '' : 'S')+' · UPDATED '+esc(root.ESPN_LAST?localDate(root.ESPN_LAST,{hour:'numeric',minute:'2-digit'}):'PENDING')+' ET</div><h1>'+esc(dateLabel)+'</h1><p>Daily probabilities, probable starters, market coverage, and game state in one view.</p></div>'
  ...
  function sourceStatus(){var list=games(),starter=0,weather=0,markets=0;list.forEach(function(g){var sp=root.SP&&root.SP[g.id],o=root.ODDS&&root.ODDS[g.id];if(sp&&sp.h&&sp.a&&sp.h.name&&sp.a.name)starter++;if(sp&&sp.wxReal)weather++;if(o&&o.real)markets++;});return [{name:'Schedule',value:root.ESPN_OK?'LIVE':'OFFLINE',status:root.ESPN_OK?'ok':'off',coverage:list.length+'/'+list.length,source:'ESPN scoreboard'}, {name:'Starters',value:starter===list.length&&list.length?'LIVE':'PARTIAL',status:starter===list.length&&list.length?'ok':'pending',coverage:starter+'/'+list.length,source:'ESPN / MLB Stats API'}, {name:'Standings',value:root.STANDINGS_OK?'LIVE':'STALE',status:root.STANDINGS_OK?'ok':'pending',coverage:root.STANDINGS_OK?'30/30':'—',source:'ESPN / MLB Stats API'}, {name:'Injuries',value:root.INJURIES&&Object.keys(root.INJURIES).length?'LIVE':'UNAVAILABLE',status:root.INJURIES&&Object.keys(root.INJURIES).length?'ok':'pending',coverage:root.INJURIES&&Object.keys(root.INJURIES).length?Object.keys(root.INJURIES).length+' reports':'—',source:'ESPN'}, {name:'Weather',value:weather===list.length&&list.length?'LIVE':weather?'PARTIAL':'UNAVAILABLE',status:weather===list.length&&list.length?'ok':'pending',coverage:weather+'/'+list.length,source:'ESPN game conditions'}, {name:'Market odds',value:markets?'LIVE':'UNAVAILABLE',status:markets?'ok':'pending',coverage:markets+'/'+list.length,source:'ESPN odds provider'}, {name:'Pitch tracking',value:list.some(function(g){return g.status==='live';})?'LIVE':'UNAVAILABLE',status:list.some(function(g){return g.status==='live';})?'ok':'pending',coverage:'Live games only',source:'MLB live feed'}];}
  ...
  root.QuantLabUI={init:init,setTab:setTab,selectGame:selectGame,openGame:openGame,enterBallpark:enterBallpark,setSlateFilter:setSlateFilter,setSlateSort:setSlateSort,toggleModelFeature:toggleModelFeature,setCamera:setCamera,toggleOverlay:toggleOverlay,setQuality:setQuality,loadPitches:loadPitches,exportSnapshots:exportSnapshots,exportTradingLog:exportTradingLog,syncSettings:syncSettings,render:renderCurrent};
